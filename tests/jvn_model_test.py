...
class TestMethods(unittest.TestCase):
    ...
    def test_jvn_vulnerability_01(self):
        ...
        description = (
            "Sun Solaris の ndd コマンドには、不正な TCP/IP のカーネルパラメータを設定されてしまう脆弱性が存在します。"
        )

        t = JvnTest()
        r = do_transaction(
            lambda db: db.query(Vulnerability).filter_by(identifier=identifier).first(),
            t,
        )
        ...
        self.assertEqual(
            r.link, "https://jvndb.jvn.jp/ja/contents/1998/JVNDB-1998-000002.html"
        )
